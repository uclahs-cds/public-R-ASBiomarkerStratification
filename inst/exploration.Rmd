...
GSA.vars <- data$biodb[, c('GSAPositives', 'BRCAMutation', 'Mutation1', 'Mutation.2')]
...
# cor(as.data.frame(lapply(data$biodb[, cor.variables], as.numeric)), use = "complete.obs", method = "spearman")
```
...
```{r}
cor(data$biodb[, c('MiPSCancerRisk', 'MiPSHighGradeCancerRisk', 'PSAHyb', 'PCA3', 'T2ERG')], use = "complete.obs", method = "spearman")
...
levels(observation.computed) <- c('MRI Positive/Biopsy Positive', 'MRI Positive/Biopsy Negative',
...
## Keeping Variables
```{r}
all.variables <- c("Record.ID", "Age", "Race", "Ethnicity", "Weight", "Height", 
...
"GSAPositives", "BRCAMutation", "Mutation1", "Mutation.2", "TNFaAverage", 
...
biokey.variables <- c("Age", "Race", "Ethnicity", "Weight", "Height", 
...
"GSAPositives", "BRCAMutation", "Mutation1", "Mutation 2", "RSInormalSignal", 
