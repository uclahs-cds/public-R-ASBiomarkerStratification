...
names(models) <- c('Demographics', 'Blood, Urine, Genetics', 'MRI Features', 'Post MRI + Biomarkers');
...
thresholds.table$group <- names(models)
...
gbm.seq.var.imp <- merge(
  ...
  biomarkers
)[, c("variable", "category", "Demographics", "Blood, Urine, Genetics", "MRI Features",
      "Post MRI + Biomarkers")]
...
kable(x = gbm.seq.var.imp,
  ...
  kable_styling()  %>%
  column_spec(2:6, width_min = "6em", width = "6em")
```
