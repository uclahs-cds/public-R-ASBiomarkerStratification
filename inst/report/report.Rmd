...
We compare the progression of the cancers by the ISUP grade groups from the previous result to the highest study result.
```{r}
...
We should the days to progression from biopsy in Figure \@ref(fig:days-bx-progression), the days to progression from diagnosis in Figure \@ref(fig:days-dx-progression), and the days to biopsy upgrade in \@ref(fig:days-dx-upgrade).
...
```{r decision-tree-prog, out.width="75%", fig.cap="Decision tree for Progressed to Treatment prediction."}
...
We should the optimal parameters for the GBM and XGB model in Table \@ref(tab:gbm-grid) and Table \@ref(tab:xgb-grid).
...
```{r}                            
xgb.grid <- list(
    nrounds = expr(seq(from = 200, to = 1000, by = 50)),
...
xgb.caption <- paste0('Hyperparameters for XGB modelfor biopsy upgraded prediction. The total grid size is: ', Reduce('*', xgb.params$length));
