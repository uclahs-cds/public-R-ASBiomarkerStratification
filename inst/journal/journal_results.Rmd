...
if(group.tests) {
  names(seq.models) <- c('Demographics', 'Blood, Urine, Genetics', 'MRI Features', 'Post MRI');
} else {
  names(seq.models) <- c('Demographics', 'Blood', 'Urine', 'Genetics', 'MRI Features', 'Post MRI');
}
...
rownames(dotmap.bg.data.int) <- col.labels[all.var.names]
...
create.multiplot(
  plot.objects = list(metric.scatterplot, bpg.dotmap),
  panel.heights = c(1, 1),
  y.relation = 'free',
  left.padding = 30,
  right.padding = 2,
