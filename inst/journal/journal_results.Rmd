...
for(i in names(uni.auc.ci)) {
  conf_int <- uni.auc.ci[[i]]
  if(conf_int[1] > 0.5) {
    count <- count + 1
    print(i)
    print(round(conf_int[c(2,1,3)], 2))
  }
}
print(count)
# create.forestplot(biodb)
```
...
```{r cutpoint-days-prog-bx, fig.cap = 'Days to progression from biopsy for the prediction groups', eval = FALSE}
...
## Cox PH
First model all of the variables
```{r}
library(mice)
# md.pattern(biodb)

tte_vars <- c("Age", "BMI", "Ethnicity", "SOCPSA", "PercentFreePSA", 
"PHI", "PCA3", "T2ERG", "MiPSHighGradeCancerRisk", "ProstateVolume",
"GeneticRiskScore", "RSIlesionSignal","PHIDensity", "PSADensity")

# Join the TTE variables and the time/status
cox.biodb <- cbind(biodb[, tte_vars], as.data.frame(surv.format(biodb$DaysDxToUpgrade, biodb$DaysDxToLastReview)))

#cor(cox.biodb[, c("SOCPSA", "PHI", "ProstateVolume", "PHIDensity", "PSADensity")], method = "spearman", use = "complete.obs")

imp.biodb <- mice(cox.biodb)
imp.biodb
# no.asian.biodb <- biodb[biodb$Race != "Asian",]

# upgrade.surv.no.asian <- do.call(
#     what = survival::Surv,
#     args = surv.format(no.asian.biodb$DaysDxToUpgrade, no.asian.biodb$DaysDxToLastReview)
#     );
...
cox_formula <- as.formula(paste0('upgrade.surv ~', paste0(tte_vars, collapse = " + ")))

# Simple cox
model <- coxph(cox_formula, data = biodb)
summary(model)

cox.mi <- with(imp.biodb, coxph(Surv(time, status) ~ Age + BMI + Ethnicity + PercentFreePSA + SOCPSA +
    PHI + PCA3 + T2ERG + MiPSHighGradeCancerRisk + GeneticRiskScore + 
    RSIlesionSignal + ProstateVolume + PHIDensity + PSADensity))
...
x <- model.matrix.lm(cox_glmformula, data = biodb, na.action = "na.omit")
