...
threshold.cost <- optimal.threshold.train(models$ProgressedToTreatment$gbm, thresholds = thresholds, cost.matrix = cost.matrix)
plot(thresholds, threshold.cost, type = "l")
lines(thresholds, moving.avg(threshold.cost, order = 5), col = 'red')

threshold.cost.rpart <- optimal.threshold.train(models$ProgressedToTreatment$rpart, thresholds = thresholds, cost.matrix = cost.matrix)
plot(thresholds, threshold.cost.rpart, type = "l")
lines(thresholds, moving.avg(threshold.cost.rpart, order = 5), col = 'red')
# thresholder(models$ProgressedToTreatment$gbm, threshold = seq(0, 1, length.out = 10))
```
...
Several metrics are computed based on the results such as $F_1$ score, area under the ROC curve (AUC-ROC), area under the precision-recall curve, accuracy, sensitivity, and specificity.
...
dotplot(bx.resamps, metric = c('F', 'AUC-ROC', 'PR-AUC', 'Accuracy', 'Sens', 'Spec'), main = 'Biopsy Upgraded Model Comparison')
```
...
rpart.plot::rpart.plot(models$BiopsyUpgraded$rpart_cost2$finalModel, main = 'Biopsy Upgraded Regression Tree FN x 2')
rpart.plot::rpart.plot(models$BiopsyUpgraded$rpart_cost3$finalModel, main = 'Biopsy Upgraded Regression Tree FN x 3')
```
...
dotplot(prog.resamps, metric = c('F', 'AUC-ROC', 'PR-AUC', 'Accuracy', 'Sens', 'Spec'), main = 'Progressed To Treatment Model Comparison')
```

```{r prog-rpart, fig.cap='Regresion tree for predicting whether the patient will progress to treatment.', out.width = '75%'}
rpart.plot::rpart.plot(models$ProgressedToTreatment$rpart_cost2$finalModel, main = 'Progressed-to-Treatment Regression Tree FN x 2')
rpart.plot::rpart.plot(models$ProgressedToTreatment$rpart_cost3$finalModel, main = 'Progressed-to-Treatment Regression Tree FN x 3')
```
...
dotplot(prostate.resamps, metric = c('F', 'AUC-ROC', 'PR-AUC', 'Accuracy', 'Sens', 'Spec'), main = 'Prostatectomy Model Comparison')
```

```{r prostate-rpart, fig.cap='Regresion tree for predicting whether the patient will require a prostatectomy.', out.width = '75%'}
rpart.plot::rpart.plot(models$Prostatectomy$rpart_cost2$finalModel, main = 'Prostatectomy Regression Tree FN x 2')
rpart.plot::rpart.plot(models$Prostatectomy$rpart_cost3$finalModel, main = 'Prostatectomy Regression Tree FN x 3')
```
...
### Progressed To Treatment
```{r}
prog.gbm.preds <- as.factor(predict(models$ProgressedToTreatment$gbm))
...
bx.gbm.preds[!is.na(biodb$BiopsyUpgraded)] <- predict(models$BiopsyUpgraded$gbm)
...
prog.model.list <- list(prog.gbm.F.model, prog.xgb.F.model, prog.rpart.F.model)
names(prog.model.list) <- c('gbm', 'xgb', 'rpart')

var.imp.joined <- compare.var.imp(prog.model.list, include.ranks = TRUE)[, -1]
                   
var.imp.joined <- var.imp.joined[order(var.imp.joined$mean.rank),]
kable(
  x = head(var.imp.joined[,c(7, 4:6)], n = 25),
