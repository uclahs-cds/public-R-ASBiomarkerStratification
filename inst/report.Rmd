...
up.models <- lapply(targets, function(tg) {
    model.file.names <- here(paste0('models/', model.names, '_', tg, '_', metric, '_', seed, '_model.RDS'));
    res <- lapply(model.file.names, readRDS);
    names(res) <- model.names;
    res
});
names(up.models) <- targets;
```
...
thresholds.info.list <- lapply(names(optimal.thresholds), function(x) {
  ...
  res <- lapply(names(target.list), function(y) {
    ...
    thres.res <- thresholder(m, threshold = threshold, final = TRUE)
    thres.res <- thres.res[statistics]
    rownames(thres.res) <- m$method
    thres.res
  })
...
To find the optimal cut point we make a false positive ()
We show how to find the optimal point for Progressed To Treatment
```{r, fig.cap="Optimal cut-points for the Progressed to Treatment prediction models"}
threshold.cost.gbm <- cost.threshold.train(models$ProgressedToTreatment$gbm, thresholds = thresholds, cost.matrix = cost.matrix)

#plot(thresholds, threshold.cost, type = "l", las = 1)
#lines(thresholds, moving.avg(threshold.cost, order = 5), col = 'red', las = 1)
...
#lines(thresholds, threshold.cost.rpart, type = "l", lty = 2, las = 1)
#lines(thresholds, moving.avg(threshold.cost.rpart, order = 5), col = 'red', lty = 2, las = 1)
...
Then we show the results for each of the models at the given optimal threshold
```{r}
