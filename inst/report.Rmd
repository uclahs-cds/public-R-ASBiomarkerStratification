...
#### Biopsy Upgraded
```{r}
bx.acc.model <- readRDS(here('models/BiopsyUpgraded_Accuracy_model.RDS'));
bx.F.model <- readRDS(here('models/BiopsyUpgraded_F_model.RDS'));

bx.resamps <- resamples(
  list(GBM.Acc = bx.acc.model$gbm.fit,
       C5.0.Acc = bx.acc.model$c50.fit,
       rpart.Acc = bx.acc.model$rpart.fit,
       GBM.F = bx.F.model$gbm.fit,
       C5.0.F = bx.F.model$c50.fit,
       rpart.F = bx.F.model$rpart.fit))

trellis.par.set(caretTheme())
dotplot(resamps, metric = c("F", "Sens", "Spec", "Accuracy"), main = "Biopsy Upgraded Model Comparision")
```

#### Progressed To Treatment
```{r}
prog.acc.model <- readRDS(here('models/ProgressedToTreatment_Accuracy_model.RDS'));
prog.F.model <- readRDS(here('models/ProgressedToTreatment_F_model.RDS'));

prog.resamps <- resamples(
  list(GBM.Acc = prog.acc.model$gbm.fit,
       C5.0.Acc = prog.acc.model$c50.fit,
       rpart.Acc = prog.acc.model$rpart.fit,
       GBM.F = prog.F.model$gbm.fit,
       C5.0.F = prog.F.model$c50.fit,
       rpart.F = prog.F.model$rpart.fit))
...
#### Progressed To Treatment
```{r}
prostate.acc.model <- readRDS(here('models/Prostatectomy_Accuracy_model.RDS'));
prostate.F.model <- readRDS(here('models/Prostatectomy_F_model.RDS'));

prostate.resamps <- resamples(
  list(GBM.Acc = prostate.acc.model$gbm.fit,
       C5.0.Acc = prostate.acc.model$c50.fit,
       rpart.Acc = prostate.acc.model$rpart.fit,
       GBM.F = prostate.F.model$gbm.fit,
       C5.0.F = prostate.F.model$c50.fit,
       rpart.F = prostate.F.model$rpart.fit))

trellis.par.set(caretTheme())
dotplot(resamps, metric = c("F", "Sens", "Spec", "Accuracy"), main = "Prostatectomy Model Comparision")

predict(prostate.F.model$c50.bestfit, newdata = biodb)
