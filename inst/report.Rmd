...
```{r variable-set, fig.cap="Variables used to predict ProgressedToTreatment and Prostatectomy"}
...
models.roc <- lapply(models, function(mods) {
  lapply(mods, function(m) {
      ...
      pROC::roc(predictor = bestPreds$yes, response = bestPreds$obs, direction = '<');
  })
})
```
...
#coords(roc.prostate.gbm, 'best');
#coords(roc.prostate.rpart, 'best');
...
$$
The second method we use to find the optimal operating point is to make a false negative FN (when we predict indolent disease when it truly is aggresive) 2 times as costly as a false positive FP.
...
cols <- c("model", "method", "threshold", "Accuracy", "Sensitivity", "Specificity", 
  "Pos Pred Value", "Neg Pred Value", "Precision", "Recall", "F1", 
  "Prevalence", "Detection Rate", "Detection Prevalence", "Balanced Accuracy"
  )

kable(thresholds.info[, cols], digits = 3, row.names = FALSE) %>%
  kable_styling()
```
