...
dotplot(bx.resamps, metric = c("F", "Sens", "Spec", "Accuracy"), main = "Biopsy Upgraded Model Comparision")
...
#### Progressed To Treatment
```{r}
xgb.F.model <- readRDS(here('models/xgb_ProgressedToTreatment_F_model.RDS'));
rpart.F.model <- readRDS(here('models/rpart_ProgressedToTreatment_F_model.RDS'));
gbm.F.model <- readRDS(here('models/gbm_ProgressedToTreatment_F_model.RDS'));

prog.resamps <- resamples(
  list(XGB.F = xgb.F.model,
       GBM.F = gbm.F.model,
       rpart.F = rpart.F.model));

dotplot(prog.resamps, metric = c("F", "Sens", "Spec", "Accuracy"), main = "Progressed To Treatment Model Comparision")

rpart.plot::rpart.plot(rpart.F.model$finalModel)
...
dotplot(prostate.resamps, metric = c("F", "Sens", "Spec", "Accuracy"), main = "Prostatectom Model Comparision")
