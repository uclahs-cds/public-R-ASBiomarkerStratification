...
kable(
  ...
  col.names = 'Variables',
  booktabs = T,
  caption = 'Variables used in prediction') %>%
...
model.lookup <- list(
  'reduced_both' = 'Both RSI Signal and PI-RADS',
  'reduced_only_RSIlesionSignal' = 'Only RSI lesion Signal',
  'reduced_only_RSIlesionPIRADS' = 'Only RSI lesion PI-RADS'
)

thresholds.table <- summary.df[, cols]
# thresholds.table$model <- unlist(model.lookup[as.character(thresholds.table$model)])
...
kable(x = bx.upgrade.var.imp[order(bx.upgrade.var.imp[,2], decreasing = TRUE), ],
  ...
  kable_styling() %>%
  column_spec(2:4, width_min = "5em", width = "5em")
```
